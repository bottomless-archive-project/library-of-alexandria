...
public interface DocumentRepository {
    ...
    @Insert("INSERT INTO documents SET id = #{id}, url = #{url}, crawled_date = NOW(), crc = #{crc}, file_size = #{fileSize}, `status` = #{status}, crawler_version = #{crawlerVersion}")
    ...
    @Update("UPDATE pdf SET file_size = #{fileSize}, crc = #{crc} WHERE id = #{id}")
    void updateFileSizeAndCrc(@Param("id") String id, @Param("fileSize") long fileSize, @Param("crc") String crc);

    @Select("SELECT * FROM pdf WHERE file_size = #{fileSize} AND crc = #{crc}")
